...
class TextMatcher:
    ...
    def match(self, pattern):
        # 将输入文本转换为模型的输入特征
        inputs = self.tokenizer(pattern, return_tensors='pt', padding=True, truncation=True).to(self.device)
        # 使用模型获取输入文本的特征表示
        with torch.no_grad():
        ...
        # 返回最相似的命令的值
        most_similar_command_value = self.text_command_pair[list(self.text_command_pair.keys())[max_similarity_index]]
        return most_similar_command_value
